...
# Tanulo és teszt halmaz letrehozasa
  n_seed=1234
...
# AutoML ranglista és legjobb modell
  head(model_automl@leaderboard,10)
  model_automl@leader
...
# LIME - Magyarazo letrehozas
  ...
  d_samp <- as.data.frame(h_test[14, features])
  # egy mintat veszunk (a 14-es kicserelheto barmelyik sorra)


# LIME - Magyarazat
  row.names(d_samp) <- "Minta 14"  # Nevadas a vizualizaciohoz
  explanations <- lime::explain(x = d_samp,
                              explainer = explainer,
                              feature_select = "auto",
                              n_features = 8) # Top x tulajdonsag
